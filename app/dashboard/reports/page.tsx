...
const BAR_GRADIENT = 'linear-gradient(to top, #02457a 0%, #02457A 100%)';
const cardCls =
...
function TrendBadge({ pct, neutral }: { pct: number; neutral?: boolean }) {
  ...
  const up = pct > 0;
  return (
    <span className={`inline-flex items-center gap-0.5 text-[11px] font-bold ${up ? 'text-[#001b48]' : 'text-[#ba1a1a]'}`}>
    ...
    <div className={`${cardCls} p-5`}>
      <div className="flex items-start justify-between gap-2">
        <div className="min-w-0">
          ...
          <p className="font-mono text-2xl font-medium text-[#02457a] dark:text-[#d6e8ee] truncate">{value}</p>
        </div>
  ...
  return (
    <div className="flex items-end justify-between gap-2 sm:gap-3 h-52 pt-8 relative">
      <div className="absolute inset-x-0 top-0 bottom-6 flex flex-col justify-between pointer-events-none">
        {[0, 1, 2, 3].map((i) => (
          <div key={i} className="border-b border-[#02457a]/[0.06] dark:border-white/5 w-full" />
        ))}
      </div>
...
function DataRows({ rows }: { rows: { label: string; hours: number; earnings: number }[] }) {
  return (
    <div className="divide-y divide-[#02457a]/[0.06] dark:divide-white/5">
      {rows.map((r) => (
        <div key={r.label} className="flex items-center justify-between py-2.5">
          ...
          <span className={`text-sm font-mono font-bold ${r.earnings > 0 ? 'text-[#02457a] dark:text-[#d6e8ee]' : 'text-gray-300 dark:text-gray-600'}`}>
            {money(r.earnings)}
          </span>
        </div>
      ))}
...
function WeeklyContent({ data, prevWeek }: { data: DayDatum[]; prevWeek: DayDatum[] }) {
  ...
  return (
    <div className="space-y-5">
      ...
      <div className={`${cardCls} p-5 border-l-[3px] border-l-[#001b48]`}>
            ...
            <p className="text-sm text-[#404752] dark:text-gray-300 leading-relaxed">
              {totalEarnings > 0 ? (
                <>
                  ...
                  <span className="font-mono font-bold text-[#02457a] dark:text-[#d6e8ee]">{best.day}</span>, earning{' '}
                  <span className="font-mono font-bold text-[#02457a] dark:text-[#d6e8ee]">{money(best.earnings)}</span> across{' '}
                  <span className="font-mono font-bold text-[#02457a] dark:text-[#d6e8ee]">{best.hours}h</span>. You&apos;re{' '}
                  {pctChange(totalEarnings, prevEarnings) >= 0 ? 'up' : 'down'}{' '}
                  <span className="font-mono font-bold text-[#02457a] dark:text-[#d6e8ee]">{Math.abs(pctChange(totalEarnings, prevEarnings))}%</span> versus last week.
                </>
              ) : (
    ...
    <DashboardLayout>
      <div className="space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
          <div>
            <h1 className="text-3xl font-extrabold text-[#02457a]">Reports</h1>
          ...
          <div className="relative">
            <button
              ...
              className="inline-flex items-center justify-center gap-2 px-5 py-3 text-white text-[11px] font-bold uppercase tracking-widest rounded-lg shadow-[0_4px_14px_rgba(2,69,122,0.25)] hover:-translate-y-0.5 transition-all disabled:opacity-60 disabled:hover:translate-y-0"
              style={primaryStyle}
            >
                  ...
                  {EXPORT_FORMATS.map((f) => {
                    ...
                    return (
                      <button
                        ...
                        className="w-full flex items-center gap-3 px-3 py-2.5 rounded-md text-sm text-[#1b1c1c] dark:text-gray-200 hover:bg-[#02457a]/[0.06] transition-colors"
                      >
                        <Icon className="h-4 w-4 text-[#02457a]" />
                        {f.label}
                      </button>
                    );
                  })}
