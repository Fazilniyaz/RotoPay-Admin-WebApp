    ...
    <>
      <style>{`
        ...
        .rp-emp-card {
          ...
          background: linear-gradient(135deg, rgba(58,146,149,0.07), rgba(16,95,104,0.07));
          border: 1px solid rgba(58,146,149,0.14);
          cursor: pointer; text-align: left; transition: background 0.15s, border-color 0.15s;
          font-family: 'Montserrat', sans-serif; overflow: hidden;
        }
        .rp-emp-card:hover { background: rgba(58,146,149,0.12); border-color: rgba(58,146,149,0.22); }
        .dark .rp-emp-card { background: rgba(200,230,226,0.07); border-color: rgba(200,230,226,0.12); }
        .dark .rp-emp-card:hover { background: rgba(200,230,226,0.12); }
        .rp-emp-avatar {
              ...
              {employers.map((emp) => {
                ...
                return (
                  <DropdownMenuItem key={emp.id} onClick={() => pick(emp.id)} className="gap-2">
                    <span className="flex h-6 w-6 items-center justify-center rounded-md bg-[#3a9295]/10 text-[10px] font-bold text-[#3a9295] dark:text-[#c8e6e2]">
                    ...
                    {isCurrent && <Check className="h-4 w-4 text-[#105f68]" />}
                  </DropdownMenuItem>
                );
              })}
            </DropdownMenuGroup>
